...
##        print("GT CEN PSEUDO SIZE")
##        print(np.size(gt_cen_pseudo))
#        # Replace this with target region?
...
#        
##        print("SIZE OF y_truth")
#        y_truth = y_pos
##        print(np.shape(y_truth))
#        y_pseudo = y_pos_pseudo
##        print("SIZE OF y_pseudo")
...
def SiamRPN_init(im, target_pos, target_sz, net, model_eval = None):
    state = dict()
    p = TrackerConfig()
    p.update(net.cfg)
    if model_eval is not None:
        p2 = TrackerConfig()
        p2.update(model_eval.cfg)
    ...
    if model_eval is not None:
        if p2.adaptive:
            if ((target_sz[0] * target_sz[1]) / float(state['im_h'] * state['im_w'])) < 0.004:
                p2.instance_size = 287  # small object big search region
            else:
                p2.instance_size = 271

            p2.score_size = (p.instance_size - p.exemplar_size) / p.total_stride + 1

        p2.anchor = generate_anchor(p.total_stride, p.scales, p.ratios, int(p.score_size))
...
def SiamRPN_track(state, im, f, last_result, att_per, def_per, image_save, iter=10, attack_mask = None, final_pos = None, im_bounds = None,use_alt_model=False):
    if use_alt_model:
        p = state['p2']
        net = state['model_eval']
    ...
    x_adv1 = rtaa_attack(net, x_crop_init, x_crop, last_result, target_pos, target_sz, scale_z, p, iteration=iter, final_pos=final_pos, im_bounds=im_bounds)
