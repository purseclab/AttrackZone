...
def rtaa_defnese(net, x_mask, x, gt, target_pos, target_sz, scale_z, p, eps=5, weight=50000, alpha=1, iteration=10, x_val_min=0, x_val_max=255):
    x = Variable(x.data)
...
def SiamRPN_track(state, im, f, last_result, att_per, def_per, image_save, iter=10):
    ...
    if type(def_per) != type(0):
        ...
        def_per = np.resize(def_per, (1, x_crop.shape[1], x_crop.shape[2], x_crop.shape[3]))
        def_per = torch.from_numpy(def_per).cuda()
    x_adv2_mask = x_adv1 + def_per * 0.01
    x_adv2_mask = torch.clamp(x_adv2_mask, 0, 255)
    x_adv2 = rtaa_defnese(net, x_adv2_mask, x_adv1, last_result, target_pos, target_sz, scale_z, p, iteration=iter)
    def_per = x_adv2 - x_adv1

    target_pos, target_sz, score = tracker_eval(net, x_adv2, target_pos, target_sz * scale_z, window, scale_z, p, f, last_result, state)
